...
// configuration step. It creates a namespace called `cpp_jam1_project`. You can modify
...
template<std::size_t Width, std::size_t Height> struct GameBoard
{
    ...
    void set(std::size_t x, std::size_t y, bool new_value)
    {
        ...
        if (new_value) { get_string(x, y) = " ON"; }
        else
        {
            get_string(x, y) = "OFF";
        }
...
int main(int argc, const char **argv)
{
 ...
 Options:
        ...
        if (args["turn_based"].asBool()) { consequence_game(); }
        else
        {
